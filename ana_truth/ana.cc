...
int main(int argc, char** argv) {
  ...
  const float angNorm =
      0.15;  // factor to increase weights of angles compare to masses
  ...
  TH2D* h_proj = new TH2D("projection", "projection", mSize, 0, (double)(mSize),
                          mSize, 0, (double)mSize);
  TH2D* h_events = new TH2D("events", "events", mSize, 0, (double)(mSize),
                            mSize, 0, (double)mSize);
  TProfile2D* h_prof =
      new TProfile2D("profile", "profile", mSize, 0, (double)(mSize), mSize, 0,
                     (double)mSize, 0, 1000);
  ...
  for (int i = 0; i < nevhisto; i++)
    h_proje[i] = new TH2D(Form("event_%02d", i), Form("event_%02d", i), mSize,
                          0, (double)(mSize), mSize, 0, (double)mSize);
  ...
  for (int h = 0; h < mSize; h++) {
    for (int w = 0; w < mSize; w++) {
      ...
      for (int i = 0; i < nevhisto; i++)
        h_proje[i]->Fill((Names2.at(i1)).c_str(), (Names1.at(i2)).c_str(), 0);
    }
  }
  ...
  for (unsigned int m = 0; m < files.size(); m++) {
	...
	if (NrLepton > 1 && bjets.size() > 1) SelectEvent = true;
      ...
      float** projArray = projectevent(CMS, maxNumber, maxTypes, missing, jets,
        ...
        for (int h = 0; h < mSize; h++) {
          ...
          for (int w = 0; w < mSize; w++) {
            if (h != w)
              ...
              // cout << std::setprecision(1) << std::scientific <<
              // float(projArray[h][w]); printf("%.2e ",
              // float(projArray[h][w]));
              const char* tt = "ok";
              cout << RED;
              printf("%.2e ", float(projArray[h][w]));
      ...
      for (int h = 0; h < mSize; h++) {
        for (int w = 0; w < mSize; w++) {
          ...
          if (event < nevhisto)
            h_proje[event]->Fill((Names2.at(i1)).c_str(),
                                 (Names1.at(i2)).c_str(), dd);

          // to get correct masses and suppress 0
          if (dd != 0) {
            h_proj->Fill((Names2.at(i1)).c_str(), (Names1.at(i2)).c_str(), dd);
            h_events->Fill((Names2.at(i1)).c_str(), (Names1.at(i2)).c_str(),
                           1.0);
