...
#name="mass_bb_zmass"
name="Mbb"
ffD=TFile("out/tev13.6pp_pythia8_ttbar_2lep_ADFilter.root")
hhD=ffD.Get(name)
cross=ffD.Get("cross");
...
def getBSM_X2hh(mass):
      ...
      ffZ=TFile("out/pythia8_X"+str(mass)+"GeV_HH2bbll_ADFilter.root")
      hh=ffZ.Get(name)
      crossZ=ffZ.Get("cross");
      #if (mass == 500): crossZ.Print("All")  
      xsecZ=crossZ.GetBinContent(1)
      lumiZ=float(crossZ.GetBinContent(4)) # secon bin is Lumi! 
...
leg2.AddEntry(mass500,"PYTHIA8 M_{X}=500-2000 scaled","lfp")
